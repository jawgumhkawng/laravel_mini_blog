   ...
   <div class="card bg-dark shadow p-4" style="border:1px solid rgb(63, 53, 53)">
       <form action="/posts/{{ $post->id }}" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control bg-dark text-white" name=name value="{{ $post->name }}" id="exampleInputEmail1" aria-describedby="emailHelp" required>
        
        </div>
        <div class="mb-3">
            <label for="exampleInputPassword1" class="form-label text-white" style="font-weight:bolder">Description</label>
            <textarea type="text" class="form-control bg-dark text-white" id=""  name=description required >{{ $post->description }}</textarea>        
        </div>
